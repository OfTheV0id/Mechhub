...
export const ProfileView: React.FC<ProfileViewProps> = ({ 
  user = { name: "张同学", avatar: "", role: "机械工程专业学生" },
  onUpdateProfile = () => {}
}) => {
  ...
  // Sync state with props if user data changes from outside (e.g. initial load)
  useEffect(() => {
    setName(user.name);
    setRole(user.role);
      ...
      >
        <motion.div variants={itemVariants} className="flex items-center justify-between mb-12">
           ...
           {isEditing ? (
             <div className="flex gap-2">
               <button onClick={handleCancel} className="p-2 rounded-full hover:bg-slate-200 text-slate-500 transition-colors">
                 <X size={20} />
               </button>
               <button onClick={handleSave} className="p-2 rounded-full bg-blue-600 text-white hover:bg-blue-700 shadow-md transition-all hover:scale-105">
                 <Save size={20} />
               </button>
             </div>
           ) : (
             <button onClick={() => setIsEditing(true)} className="p-2 rounded-full hover:bg-slate-200 text-slate-500 transition-colors">
               <Edit2 size={20} />
             </button>
           )}
        </motion.div>
        ...
        >
          ...
          <div className="relative group cursor-pointer mb-8">
            <div className={`w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-xl ring-1 ring-slate-100 relative ${isEditing ? 'ring-blue-400 ring-4' : ''}`}>
              <img 
                src={avatar} 
                alt="Profile" 
          ...
          <div className="w-full max-w-2xl grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-700 ml-1">职业 / 学位</label>
              <input 
                type="text" 
                value={role}
...
const SkillCard = ({ icon, title, status, delay }: { icon: React.ReactNode, title: string, status: 'earned' | 'locked', delay: number }) => (
  <motion.div 
  ...
  >
    ...
    <div className={`w-16 h-16 rounded-2xl flex-shrink-0 flex items-center justify-center text-3xl relative z-10 transition-colors ${
       status === 'earned' ? 'bg-slate-50 text-slate-800 group-hover:bg-blue-50 group-hover:text-blue-600' : 'bg-slate-100 text-slate-400'
    }`}>
      {icon}
    </div>
...
const TimelineNode = ({ x, y, title, status, delay, isTop }: { x: number, y: number, title: string, status: 'completed' | 'current' | 'locked', delay: number, isTop?: boolean }) => {
  ...
  return (
    ...
    >
      <div className={`w-10 h-10 rounded-full flex items-center justify-center border-[4px] z-10 shadow-sm relative ${
         status === 'completed' ? 'bg-white border-blue-500' :
         status === 'current' ? 'bg-blue-600 border-blue-200 ring-4 ring-blue-100' :
         'bg-slate-100 border-slate-200'
      }`}>
         {status === 'completed' && <div className="w-3 h-3 bg-blue-500 rounded-full" />}
